...
function updateAcceleration2(bodies) {
    ...
    for (const [i, bod1] of bodiesArr.entries()) {
        for (let j = i + 1; j < bodiesArr.length; j++) {
            ...
            if (distance < (b1.r + b2.r) * 300 && !bodysInCollisionsList(collisions, b1, b2)) {
                // collide
                b1.acc = Vec2.zero;
                b2.acc = Vec2.zero;
        ...
        if (!Simulation.instance) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const body = new DynamicBody({
                    pos: pos,
                    vel: new Vec2(pos.y / distanceFromOrigin * 250000, -pos.x / distanceFromOrigin * 250000),
                });
